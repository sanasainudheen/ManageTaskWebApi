    ...
    [HttpPost("CreateGroup")]
        public async Task<IActionResult> CreateGroup([FromBody] Group groupModel)
        {
            ...
            else if (id == -1)
            {
                return Ok(new UserManagerResponse
                {
                    Message = "The given group name is already existing.Please Change",
                    IsSuccess = false,
                });
            }
            else
        ...
        [HttpGet("GetStatusCount/{flag}")]
        public async Task<ActionResult> GetStatusCount(int flag)
        {
            var records = await _taskRepository.GetStatusCount(flag);
            //return Ok(new UserManagerResponse
            //{
            //    Message = "Task has been assigned to the group successfully.",
            //    IsSuccess = true,
