...
namespace TaskManagerWebApi.Repository
{
    public class TaskRepository:ITaskRepository
    {
        ...
        {
            var record=_context.Groups.FirstOrDefault(x => x.GroupName == groupModel.GroupName);
            if (record == null)
            {
                var group = new Group
                {
        ...
        public async Task<List<StatusCount>>  GetStatusCount(int flag)
        {
            ...
            var records = await _context.StatusCounts.FromSqlRaw("EXEC Sp_StatusCount @flag", parameters.ToArray()).ToListAsync();
            return records;
        }
